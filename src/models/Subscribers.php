...
class Subscribers extends Main {
    public function selectAccountDetails(int $subscriberId) {
        $db = $this->dbConnect();
        $selectAccountDetailsQuery = "SELECT sub.first_subscription_date, sl.program_type, sl.date FROM subscribers sub LEFT JOIN subscription_logs sl ON sub.user_id = sl.user_id WHERE sub.user_id = ? ORDER BY sl.date DESC";
    ...
    public function selectSubscriberDetails(int $subscriberId) {
        $db = $this->dbConnect();
        $selectSubscriberDetailsQuery = "SELECT CONCAT(a.first_name, ' ', UPPER(a.last_name)) as 'name', sub.user_id, DATE_FORMAT(FROM_DAYS(DATEDIFF(NOW(), usd.birthdate)), '%Y') + 0 AS 'age', usd.program_goal, udd.job_style, sub.program_status, usd.height, usd.initial_weight, usd.food_restrictions, usd.food_intolerances, usd.sport_habits, usd.daily_meals, usd.snacks_enabled, uwr.date, uwr.weight AS 'current_weight', usd.weight_goal FROM subscribers sub INNER JOIN accounts a ON sub.user_id = a.id INNER JOIN applications app ON sub.user_id = app.user_id INNER JOIN users_static_data usd ON app.user_id = usd.user_id INNER JOIN users_dynamic_data udd ON sub.user_id = udd.user_id LEFT JOIN users_weight_reports uwr ON sub.user_id = uwr.user_id WHERE sub.user_id = ? ORDER BY uwr.date DESC";
    ...
    public function selectSubscriberData (int $subscriberId) {
        $db = $this->dbConnect();
        $selectSubscriberDataQuery = "SELECT email, first_name FROM accounts a INNER JOIN subscribers sub ON a.id = sub.user_id WHERE sub.user_id = ?";
    ...
    public function selectSubscriberHeader(int $subscriberId) {
        $db = $this->dbConnect();
        $selectSubscriberHeaderQuery = "SELECT CONCAT(a.first_name, ' ', UPPER(a.last_name)) as 'name', usd.daily_meals, usd.snacks_enabled FROM subscribers sub INNER JOIN accounts a ON sub.user_id = a.id INNER JOIN users_static_data usd ON sub.user_id = usd.user_id WHERE sub.user_id = ?";
