...
class ApplianceResponder extends Mailer {
    ...
    public function sendApprovalNotification(array $applicantData) {
        return mail($applicantData['email'], $this->_SUBJECT, $this->_getApprovalMessage($applicantData), parent::HEADERS);
    }
    
    public function sendRejectionNotification(string $messageType, array $applicantData) {
        return 
            $messageType === 'default'
            ? (mail($applicantData['email'], $this->_SUBJECT, $this->_getDefaultRejectionMessage($applicantData), parent::HEADERS))
            : (mail($applicantData['email'], $this->_SUBJECT, $this->_getCustomRejectionMessage($applicantData), parent::HEADERS));
    }
